...
<main class="max-w-7xl mx-auto px-6 py-12">
    ...
    <section class="mt-40 border-t border-slate-100 pt-24 pb-40">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-10 mb-20">
                <div class="max-w-sm">
                    <h3 class="text-3xl font-black text-slate-900 tracking-tight mb-4">Ulasan <span class="text-teal-600">Pelanggan.</span></h3>
                    <p class="text-slate-500 text-sm font-medium leading-relaxed">Apa yang mereka katakan tentang kualitas serat dan tekstur material ini.</p>
                </div>
                
                <div class="flex items-center gap-8 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex-1 md:max-w-md">
                    <div class="text-center px-4 border-r border-slate-100">
                        <span class="text-5xl font-black text-slate-900 tracking-tighter italic">{{ $stats['average'] }}</span>
                        <div class="flex items-center gap-0.5 mt-2 justify-center">
                            @for($i=1; $i<=5; $i++)
                                <svg class="w-3 h-3 {{ $i <= round($stats['average']) ? 'text-amber-400' : 'text-slate-100' }} fill-current" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                            @endfor
                        </div>
                        <p class="text-[8px] font-black text-slate-300 uppercase tracking-[0.2em] mt-3">{{ $stats['total'] }} Reviews</p>
                    </div>
                        ...
                        @foreach([5,4,3,2,1] as $star)
                            <div class="flex items-center gap-3 text-[9px] font-black text-slate-400">
                                ...
                                <div class="flex-1 h-1 bg-slate-50 rounded-full overflow-hidden">
                                    <div class="h-full bg-teal-500 rounded-full" style="width: {{ $stats['percentages'][$star] }}%"></div>
                                </div>
                                <span class="w-6 text-right">{{ $stats['percentages'][$star] }}%</span>
                            </div>
                        @endforeach
                    </div>
                </div>
            ...
            <div class="space-y-6">
                @forelse($product->reviews as $review)
                    <div class="bg-white p-10 rounded-[3rem] border border-slate-50 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-500 group/review">
                        <div class="flex items-start justify-between mb-8">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-2xl bg-slate-100 flex items-center justify-center font-black text-slate-400 uppercase text-sm group-hover/review:bg-teal-600 group-hover/review:text-white transition-all duration-500">
                                    {{ substr($review->user->name, 0, 1) }}
                                </div>
                                <div>
                                    <h4 class="text-sm font-black text-slate-900 uppercase tracking-tight">{{ $review->user->name }}</h4>
                                    <div class="flex items-center gap-3 mt-1">
                                        <div class="flex items-center">
                                            @for($i=1; $i<=5; $i++)
                                                <svg class="w-2.5 h-2.5 {{ $i <= $review->rating ? 'text-amber-400' : 'text-slate-100' }} fill-current" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                                            @endfor
                                        </div>
                                        <span class="text-[9px] text-slate-300 font-bold uppercase tracking-widest">{{ $review->created_at->diffForHumans() }}</span>
                                    </div>
                                </div>
                            </div>

                            @if($review->order_id)
                                <div class="flex items-center gap-1.5 px-3 py-1 bg-teal-50 text-teal-600 rounded-lg border border-teal-100">
                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/></svg>
                                    <span class="text-[8px] font-black uppercase tracking-widest">Verified Purchase</span>
                                </div>
                            @endif
                        </div>

                        <p class="text-slate-600 leading-relaxed text-base font-medium italic">
                            "{{ $review->comment }}"
                        </p>
                        ...
                        @if($review->image)
                            <div class="mt-8 flex gap-4">
                                <a href="{{ asset('storage/' . $review->image) }}" class="glightbox" data-gallery="review-{{ $review->id }}">
                                    <div class="relative group/img overflow-hidden rounded-2xl w-32 h-32 border border-slate-100">
                                        <img src="{{ asset('storage/' . $review->image) }}" class="w-full h-full object-cover transition-transform duration-700 group-hover/img:scale-110">
                                        <div class="absolute inset-0 bg-black/5 group-hover/img:bg-transparent transition-all"></div>
                                    </div>
                                </a>
                            </div>
                        @endif
                ...
                @empty
                    <div class="py-24 text-center bg-slate-50/50 rounded-[3.5rem] border border-dashed border-slate-200">
                        ...
                        <p class="text-slate-400 font-black uppercase tracking-[0.3em] text-[10px]">Belum ada ulasan untuk material ini</p>
                    </div>
                @endforelse
            </div>
        </section>
