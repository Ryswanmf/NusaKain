    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 w-full">
            <div class="min-w-0">
                ...
                <p class="text-[11px] md:text-sm text-slate-500 font-medium mt-0.5 truncate text-teal-600 md:text-slate-500">Perbarui informasi akun {{ $user->name }}.</p>
            </div>
    ...
    <div class="py-2">
        <form action="{{ route('admin.users.update', $user) }}" method="POST">
            ...
            <div class="max-w-3xl bg-white p-8 md:p-12 rounded-[2rem] md:rounded-[3rem] border border-slate-100 shadow-sm space-y-6 md:space-y-8">
                ...
                <div class="p-6 bg-amber-50 rounded-2xl md:rounded-3xl border border-amber-100">
                    <p class="text-xs font-bold text-amber-700 leading-relaxed italic">Biarkan kosong jika tidak ingin mengubah password.</p>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                    <div class="space-y-2 md:space-y-3">
                        <label class="text-[10px] md:text-xs font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Password Baru</label>
