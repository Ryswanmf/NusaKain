    ...
    <x-slot name="header">
        <div class="flex items-center space-x-5">
            ...
            <div>
                ...
                <p class="text-sm text-slate-500 font-medium mt-1">Daftarkan brand fashion yang berkolaborasi.</p>
            </div>
        </div>
    </x-slot>

    <div class="py-2">
        <form action="{{ route('admin.landing.partners.store') }}" method="POST">
            @csrf
                    ...
                    <div class="bg-white p-10 md:p-12 rounded-[3rem] border border-slate-100 shadow-sm space-y-8">
                        <div class="space-y-3">
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" required
                                class="w-full px-8 py-5 bg-slate-50 border-none rounded-[2rem] focus:ring-2 focus:ring-teal-600 focus:bg-white transition-all font-bold text-slate-900 placeholder-slate-300" placeholder="Contoh: EIGER, ERIGO">
                        </div>
