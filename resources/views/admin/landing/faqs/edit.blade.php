    ...
    <x-slot name="header">
        <div class="flex items-center space-x-5">
            ...
            <div>
                ...
                <p class="text-sm text-slate-500 font-medium mt-1">Perbarui jawaban untuk audiens Anda.</p>
            </div>
        </div>
    </x-slot>

    <div class="py-2">
        <form action="{{ route('admin.landing.faqs.update', $faq->id) }}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    <div class="bg-white p-10 md:p-12 rounded-[3rem] border border-slate-100 shadow-sm space-y-8">
                        ...
                        <div class="space-y-3">
                            ...
                            <textarea name="answer" rows="6" required
                                class="w-full px-8 py-6 bg-slate-50 border-none rounded-[2rem] focus:ring-2 focus:ring-teal-600 focus:bg-white transition-all font-medium text-slate-600 leading-relaxed placeholder-slate-300">{{ old('answer', $faq->answer) }}</textarea>
                        </div>
                    </div>
                </div>
