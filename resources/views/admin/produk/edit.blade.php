<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-3 md:space-x-5 w-full">
            <a href="{{ route('admin.produk.index') }}" class="w-10 h-10 md:w-12 h-12 flex items-center justify-center bg-white border border-slate-100 text-slate-400 hover:text-teal-600 rounded-xl md:rounded-2xl transition-all shadow-sm active:scale-90 flex-shrink-0">
            ...
            <div class="min-w-0">
                ...
                <p class="hidden xs:block text-[11px] md:text-sm text-slate-500 font-medium mt-0.5">ID: #{{ str_pad($produk->id, 4, '0', STR_PAD_LEFT) }}</p>
            </div>
        </div>
    </x-slot>
                ...
                <div class="lg:col-span-2 space-y-6 md:space-y-10">
                    <div class="bg-white p-6 md:p-12 rounded-[2rem] md:rounded-[3rem] border border-slate-100 shadow-sm space-y-6 md:space-y-8">
                        <div class="space-y-2 md:space-y-3">
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $produk->name) }}" required
                                class="w-full px-5 py-4 md:px-8 md:py-5 bg-slate-50 border-none rounded-xl md:rounded-[2rem] focus:ring-2 focus:ring-teal-600 focus:bg-white transition-all font-bold text-slate-900 text-sm md:text-base" placeholder="Nama produk">
                        ...
                        <div class="space-y-2 md:space-y-3">
                            ...
                            <textarea name="description" id="description" rows="6" md:rows="8"
                                class="w-full px-5 py-4 md:px-8 md:py-6 bg-slate-50 border-none rounded-xl md:rounded-[2rem] focus:ring-2 focus:ring-teal-600 focus:bg-white transition-all font-medium text-slate-600 text-sm md:text-base leading-relaxed" placeholder="Detail produk...">{{ old('description', $produk->description) }}</textarea>
                                ...
                                <div class="variant-item p-6 bg-slate-50 rounded-[1.5rem] border border-slate-100 relative group animate__animated animate__fadeIn">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                        <div class="space-y-1">
                                            ...
                                            <input type="text" name="variants[{{ $index }}][name]" value="{{ $variant->name }}" required class="w-full px-4 py-3 bg-white border-none rounded-xl focus:ring-2 focus:ring-teal-600 font-bold text-slate-900 text-sm">
                                        </div>
                                        <div class="space-y-1">
                                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">SKU</label>
                                            <input type="text" name="variants[{{ $index }}][sku]" value="{{ $variant->sku }}" class="w-full px-4 py-3 bg-white border-none rounded-xl focus:ring-2 focus:ring-teal-600 font-bold text-slate-900 text-sm">
                                        </div>
                                        <div class="space-y-1">
                                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Harga (Opsional)</label>
                                            <input type="number" name="variants[{{ $index }}][price]" value="{{ $variant->price }}" class="w-full px-4 py-3 bg-white border-none rounded-xl focus:ring-2 focus:ring-teal-600 font-bold text-slate-900 text-sm">
                                        </div>
                                        <div class="space-y-1">
                                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Stok</label>
                                            <input type="number" name="variants[{{ $index }}][stock]" value="{{ $variant->stock }}" required class="w-full px-4 py-3 bg-white border-none rounded-xl focus:ring-2 focus:ring-teal-600 font-bold text-slate-900 text-sm">
                                        </div>
                                        <div class="space-y-1">
                                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Berat (gr)</label>
                                            <input type="number" name="variants[{{ $index }}][weight]" value="{{ $variant->weight }}" class="w-full px-4 py-3 bg-white border-none rounded-xl focus:ring-2 focus:ring-teal-600 font-bold text-slate-900 text-sm">
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                        ...
                        <template id="variant-template">
                            <div class="variant-item p-6 bg-slate-50 rounded-[1.5rem] border border-slate-100 relative group animate__animated animate__fadeIn">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                    ...
                                    <div class="space-y-1">
                                        <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">SKU</label>
                                        <input type="text" name="variants[INDEX][sku]" placeholder="SKU" class="w-full px-4 py-3 bg-white border-none rounded-xl focus:ring-2 focus:ring-teal-600 font-bold text-slate-900 text-sm">
                                    </div>
                                    <div class="space-y-1">
                                        <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Harga (Opsional)</label>
                                        <input type="number" name="variants[INDEX][price]" placeholder="Sama" class="w-full px-4 py-3 bg-white border-none rounded-xl focus:ring-2 focus:ring-teal-600 font-bold text-slate-900 text-sm">
                                    </div>
                                    <div class="space-y-1">
                                        <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Stok</label>
                    ...
                    <div class="bg-white p-6 md:p-12 rounded-[2rem] md:rounded-[3rem] border border-slate-100 shadow-sm">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mt-6">
                            ...
                            <div class="space-y-2 md:space-y-3">
                                ...
                                <div class="relative">
                                    <input type="number" name="weight" id="weight" value="{{ old('weight', $produk->weight) }}" required min="1"
                ...
                <div class="space-y-6 md:space-y-10">
                    <div class="bg-white p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] border border-slate-100 shadow-sm space-y-6 md:space-y-8">
                        <div class="space-y-2 md:space-y-3">
                            ...
                            <input type="text" name="category" id="category" value="{{ old('category', $produk->category) }}"
                                class="w-full px-5 py-4 md:px-8 md:py-5 bg-slate-50 border-none rounded-xl md:rounded-[2rem] focus:ring-2 focus:ring-teal-600 focus:bg-white transition-all font-bold text-slate-900 text-sm md:text-base" placeholder="Kategori">
                        ...
                        <div class="pt-2 md:pt-4 flex items-center justify-between px-2">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ $produk->is_active ? 'checked' : '' }}>
                    ...
                    <div class="bg-white p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] border border-slate-100 shadow-sm space-y-4 md:space-y-6">
                        ...
                        <div class="relative group aspect-square bg-slate-50 rounded-2xl md:rounded-[2.5rem] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center overflow-hidden transition-all hover:border-teal-400 hover:bg-teal-50/30">
                            ...
                            @if($produk->image)
                                <img id="preview" src="{{ asset('storage/' . $produk->image) }}" class="absolute inset-0 w-full h-full object-cover">
                            @else
                                <img id="preview" class="hidden absolute inset-0 w-full h-full object-cover">
                            @endif
                            <div id="upload-placeholder" class="{{ $produk->image ? 'hidden' : '' }} text-center p-4">
