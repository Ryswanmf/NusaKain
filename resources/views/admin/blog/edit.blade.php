    ...
    <x-slot name="header">
        <div class="flex items-center space-x-3 md:space-x-5 w-full">
            ...
            <div class="min-w-0">
                ...
                <p class="hidden xs:block text-[11px] md:text-sm text-slate-500 font-medium mt-0.5 truncate">{{ $blog->title }}</p>
            </div>
        </div>
    </x-slot>
                    ...
                    <div class="bg-white p-6 md:p-12 rounded-[2rem] md:rounded-[3rem] border border-slate-100 shadow-sm space-y-6 md:space-y-8">
                        <div class="space-y-2 md:space-y-3">
                            ...
                            <input type="text" name="title" value="{{ old('title', $blog->title) }}" required
                                class="w-full px-5 py-4 md:px-8 md:py-5 bg-slate-50 border-none rounded-xl md:rounded-[2rem] focus:ring-2 focus:ring-teal-600 focus:bg-white transition-all font-bold text-slate-900 text-sm md:text-base placeholder-slate-300">
                        </div>

                        <div class="space-y-2 md:space-y-3">
                            <label class="text-[10px] md:text-xs font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Ringkasan</label>
                            <textarea name="excerpt" rows="3"
                                class="w-full px-5 py-4 md:px-8 md:py-5 bg-slate-50 border-none rounded-xl md:rounded-[2rem] focus:ring-2 focus:ring-teal-600 focus:bg-white transition-all font-medium text-slate-600 text-sm md:text-base leading-relaxed placeholder-slate-300">{{ old('excerpt', $blog->excerpt) }}</textarea>
                        </div>
                        ...
                        <div class="space-y-2 md:space-y-3">
                            ...
                            <textarea name="content" rows="12" md:rows="15" required
                                class="w-full px-5 py-4 md:px-8 md:py-6 bg-slate-50 border-none rounded-xl md:rounded-[2rem] focus:ring-2 focus:ring-teal-600 focus:bg-white transition-all font-medium text-slate-600 text-sm md:text-base leading-relaxed placeholder-slate-300">{{ old('content', $blog->content) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6 md:space-y-10">
                    <div class="bg-white p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] border border-slate-100 shadow-sm space-y-6 md:space-y-8">
                        ...
                        <div class="space-y-2 md:space-y-3">
                            ...
                            <input type="date" name="published_at" value="{{ old('published_at', $blog->published_at ? $blog->published_at->format('Y-m-d') : '') }}"
                    ...
                    <div class="bg-white p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] border border-slate-100 shadow-sm space-y-4 md:space-y-6">
                        ...
                        <div class="relative group aspect-video bg-slate-50 rounded-2xl md:rounded-[2.5rem] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center overflow-hidden transition-all hover:border-teal-400 hover:bg-teal-50/30">
                            ...
                            @if($blog->image)
                                <img id="preview" src="{{ asset('storage/' . $blog->image) }}" class="absolute inset-0 w-full h-full object-cover">
                            @else
                                <img id="preview" class="hidden absolute inset-0 w-full h-full object-cover">
                            @endif
                            <div id="upload-placeholder" class="{{ $blog->image ? 'hidden' : '' }} text-center p-4">
