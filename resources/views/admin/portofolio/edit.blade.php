    ...
    <x-slot name="header">
        <div class="flex items-center space-x-3 md:space-x-5 w-full">
            ...
            <div class="min-w-0">
                ...
                <p class="hidden xs:block text-[11px] md:text-sm text-slate-500 font-medium mt-0.5 truncate">{{ $portofolio->title }}</p>
            </div>
        </div>
    </x-slot>
                ...
                <div class="lg:col-span-2 space-y-6 md:space-y-10">
                    <div class="bg-white p-6 md:p-12 rounded-[2rem] md:rounded-[3rem] border border-slate-100 shadow-sm space-y-6 md:space-y-8">
                        <div class="space-y-2 md:space-y-3">
                            ...
                            <input type="text" name="title" value="{{ old('title', $portofolio->title) }}" required
                                class="w-full px-5 py-4 md:px-8 md:py-5 bg-slate-50 border-none rounded-xl md:rounded-[2rem] focus:ring-2 focus:ring-teal-600 focus:bg-white transition-all font-bold text-slate-900 text-sm md:text-base" placeholder="Judul">
                        ...
                        <div class="space-y-2 md:space-y-3">
                            ...
                            <textarea name="description" rows="6" md:rows="8"
                                class="w-full px-5 py-4 md:px-8 md:py-6 bg-slate-50 border-none rounded-xl md:rounded-[2rem] focus:ring-2 focus:ring-teal-600 focus:bg-white transition-all font-medium text-slate-600 text-sm md:text-base leading-relaxed" placeholder="Narasi...">{{ old('description', $portofolio->description) }}</textarea>
                    ...
                    <div class="bg-white p-6 md:p-12 rounded-[2rem] md:rounded-[3rem] border border-slate-100 shadow-sm">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                            <div class="space-y-2 md:space-y-3">
                                ...
                                <input type="text" name="client_name" value="{{ old('client_name', $portofolio->client_name) }}"
                            ...
                            <div class="space-y-2 md:space-y-3">
                                ...
                                <input type="date" name="project_date" value="{{ old('project_date', $portofolio->project_date ? $portofolio->project_date->format('Y-m-d') : '') }}"
                ...
                <div class="space-y-6 md:space-y-10">
                    <div class="bg-white p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] border border-slate-100 shadow-sm space-y-6 md:space-y-8">
                        <div class="space-y-2 md:space-y-3">
                            ...
                            <input type="text" name="category" value="{{ old('category', $portofolio->category) }}"
                        ...
                        <div class="pt-2 md:pt-4 flex items-center justify-between px-2">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="is_published" value="1" class="sr-only peer" {{ $portofolio->is_published ? 'checked' : '' }}>
                    ...
                    <div class="bg-white p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] border border-slate-100 shadow-sm space-y-4 md:space-y-6">
                        ...
                        <div class="relative group aspect-video bg-slate-50 rounded-2xl md:rounded-[2.5rem] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center overflow-hidden transition-all hover:border-teal-400 hover:bg-teal-50/30">
                            ...
                            @if($portofolio->image)
                                <img id="preview" src="{{ asset('storage/' . $portofolio->image) }}" class="absolute inset-0 w-full h-full object-cover">
                            @else
                                <img id="preview" class="hidden absolute inset-0 w-full h-full object-cover">
                            @endif
                            <div id="upload-placeholder" class="{{ $portofolio->image ? 'hidden' : '' }} text-center p-4">
