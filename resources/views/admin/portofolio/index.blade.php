    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 w-full">
            <div class="min-w-0">
                ...
                <p class="text-[11px] md:text-sm text-slate-500 font-medium mt-0.5 truncate">Dokumentasi karya dan kolaborasi Nusakain.</p>
            </div>
            <a href="{{ route('admin.portofolio.create') }}" class="inline-flex items-center justify-center px-6 py-2.5 md:px-8 md:py-3 text-[12px] md:text-[14px] font-bold text-white bg-slate-900 rounded-xl md:rounded-2xl hover:bg-teal-600 transition-all shadow-xl shadow-slate-200 hover:shadow-teal-100 active:scale-95 self-start md:self-auto">
                ...
                <table class="w-full text-left border-collapse min-w-[600px] md:min-w-full">
                    <thead>
                        <tr class="text-[9px] md:text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] bg-slate-50/50">
                            <th class="px-4 md:px-8 py-4 md:py-6">Project & Client</th>
                                ...
                                <td class="px-4 md:px-8 py-4 md:py-6">
                                    <div class="flex items-center space-x-3 md:space-x-5">
                                        <div class="relative w-14 h-10 md:w-20 md:h-14 bg-slate-100 rounded-lg md:rounded-xl overflow-hidden flex-shrink-0 shadow-inner">
                                            @if($portfolio->image)
                                                <img src="{{ asset('storage/' . $portfolio->image) }}" alt="{{ $portfolio->title }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                                            @else
                                        ...
                                        <div class="min-w-0">
                                            <div class="text-[13px] md:text-[15px] font-black text-slate-900 leading-tight truncate">{{ $portfolio->title }}</div>
                                            <div class="text-[9px] md:text-[11px] text-slate-400 font-bold mt-0.5 md:mt-1 truncate italic uppercase">{{ $portfolio->client_name ?? 'Internal' }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="hidden sm:table-cell px-4 md:px-8 py-4 md:py-6">
                                    <span class="px-3 py-1 bg-slate-100 text-slate-600 text-[9px] md:text-[11px] font-black rounded-full uppercase tracking-wider">{{ $portfolio->category ?? 'Fashion' }}</span>
                                </td>
                                <td class="hidden md:table-cell px-4 md:px-8 py-4 md:py-6 text-[13px] font-bold text-slate-600 whitespace-nowrap">
                                    {{ $portfolio->project_date ? $portfolio->project_date->format('M Y') : '-' }}
                                </td>
                                <td class="px-4 md:px-8 py-4 md:py-6 text-center">
                                    <div class="flex flex-col items-center">
                                        <div class="w-1.5 h-1.5 md:w-2 md:h-2 {{ $portfolio->is_published ? 'bg-teal-500' : 'bg-slate-300' }} rounded-full mb-1"></div>
                                        <span class="text-[9px] md:text-[10px] font-black {{ $portfolio->is_published ? 'text-teal-600' : 'text-slate-400' }} uppercase tracking-widest">{{ $portfolio->is_published ? 'Live' : 'Draft' }}</span>
                                    </div>
                                </td>
                                <td class="px-4 md:px-8 py-4 md:py-6 text-right">
                                    <div class="flex items-center justify-end space-x-1 md:space-x-3">
                                        <a href="{{ route('admin.portofolio.edit', $portfolio->id) }}" class="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center bg-white border border-slate-100 text-blue-600 hover:bg-blue-600 hover:text-white rounded-lg md:rounded-xl transition-all shadow-sm active:scale-90">
                                        ...
                                        <form action="{{ route('admin.portofolio.destroy', $portfolio->id) }}" method="POST" onsubmit="return confirm('Hapus karya ini?')">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <tr>
                                <td colspan="5" class="px-8 py-24 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <h3 class="text-base md:text-lg font-black text-slate-900 tracking-tight">Belum Ada Karya</h3>
                                        <p class="text-xs md:text-sm text-slate-400 font-medium mt-1">Mulailah mengunggah hasil kolaborasi brand Anda.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
